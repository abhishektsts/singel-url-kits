...
{capture name=path}
	<a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'htmlall':'UTF-8'}" rel="nofollow" title="{l s='Go back to the Checkout' mod='Worldline'}">{l s='Checkout' mod='Worldline'}</a><span class="navigation-pipe">{$navigationPipe}</span>{l s='Pay using Worldline' mod='Worldline'}
{/capture}
