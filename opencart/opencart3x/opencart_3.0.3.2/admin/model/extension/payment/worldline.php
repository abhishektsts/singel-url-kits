	...
	public function add($merchant_details)
	{
		if (count($merchant_details) > 0) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "worldline`(
				...
				VALUES (
				'" . trim($merchant_details['worldline_merchant_code']) . "', 
				'" . $merchant_details['payment_worldline_request_type'] . "', 
				'" . trim($merchant_details['payment_worldline_key']) . "',
				'" . trim($merchant_details['payment_worldline_iv']) . "', 
				'" . $merchant_details['payment_worldline_webservice_locator'] . "', 
				'" . $merchant_details['payment_worldline_order_status'] . "', 
	...
	public function edit($merchant_details)
	{
		if (count($merchant_details) > 0) {
			$this->db->query("UPDATE `" . DB_PREFIX . "worldline`	
				SET 
				`merchant_code` = '" . trim($merchant_details['worldline_merchant_code']) . "',
				`request_type` = '" . $merchant_details['payment_worldline_request_type'] . "',
				`key` = '" . trim($merchant_details['payment_worldline_key']) . "',
				`iv` = '" . trim($merchant_details['payment_worldline_iv']) . "',
				`webservice_locator` = '" . $merchant_details['payment_worldline_webservice_locator'] . "',
				`order_status` = '" . $merchant_details['payment_worldline_order_status'] . "',
