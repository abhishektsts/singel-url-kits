...
function woocommerce_worldline_init()
{
	...
	class WC_worldline extends WC_Payment_Gateway
	{
		...
		public function __construct()
		{
			...
			$this->notify_url = str_replace('https:', 'https:', add_query_arg('wc-api', 'WC_worldline', home_url('/'))); //$woocommerce->api_request_url( 'WC_worldline' );
			$this->msg['message'] = "";
			$this->msg['class']   = "";
...
<?php
		...
		public function generate_worldline_form($order_id)
		{
			...
			$findme   = '+91';
			$pos = strpos($order->billing_phone, $findme);
			if ($pos === false) {
				$transactionRequestBean->setMobileNumber($order->billing_phone);
			} else {

				$updatedMob = substr($order->billing_phone, 3);
				$transactionRequestBean->setMobileNumber($updatedMob);
			}
			$transactionRequestBean->setOrderid($woocommerce->session->order_id);
